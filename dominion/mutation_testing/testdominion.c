...
void printContents(struct gameState* g) {
	int i;
	char cardName[256];
	int curPlayer = whoseTurn(g);
	int handSize = g->handCount[curPlayer];
	int deckSize = g->deckCount[curPlayer];
	printf("\nPlayer %d's Turn.\n\n", curPlayer);
	printf("Hand:\n");
	
	for (i = 0; i < handSize; i++) {
		cardNumToName(g->hand[curPlayer][i], cardName);
		printf("%d. %s\n", (i + 1), cardName);
	}
	...
	for (i = 0; i < deckSize; i++) {
		cardNumToName(g->deck[curPlayer][i], cardName);
		printf("%d. %s\n", (i + 1), cardName);
	}
