...
int mine_call(struct gameState* state, int currentPlayer, int choice1, int choice2, int handPos, int i, int j) {
	j = state->hand[currentPlayer][choice1];  //store card we will trash

    if (state->hand[currentPlayer][choice1] < copper || state->hand[currentPlayer][choice1] > gold)
		return -1;
		
    if (choice1 > treasure_map || choice2 < curse)
		return -1;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
      return mine_call(state, currentPlayer, choice1, choice2, handPos, i, j);
